...
void WalletTxToJSON(const CWalletTx &wtx, UniValue &entry) {
    ...
    BOOST_FOREACH(const uint256 &conflict, wtx.GetConflicts())conflicts.push_back(conflict.GetHex());
                                    ...
                                    addressInfo.push_back(ValueFromAmount(balances[address]));
                                    {
                                        if (pwalletMain->mapAddressBook.find(CBitcoinAddress(address).Get()) !=
                                            ...
                                            addressInfo.push_back(pwalletMain->mapAddressBook.find(
                                                    CBitcoinAddress(address).Get())->second.name);
                                    }
...
UniValue getbalance(const UniValue &params, bool fHelp) {
    ...
    if (params[0].get_str() == "*") {
        ...
        for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
            ...
            if (wtx.GetDepthInMainChain() >= nMinDepth) {
                BOOST_FOREACH(const COutputEntry &r, listReceived)nBalance += r.amount;
            }
            BOOST_FOREACH(const COutputEntry &s, listSent)nBalance -= s.amount;
            nBalance -= allFee;
        }
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
                ...
                + HelpRequiringPassphrase() + "\n"
                                              ...
                                              "    {\n"
                                              "      \"address\":amount   (numeric or string) The zoin address is the key, the numeric amount (can be string) in " +
                ...
                + HelpExampleCli("sendmany",
                                 "\"\" \"{\\\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\" 6 \"testing\"") +
                "\nSend two amounts to two different addresses, subtract fee from amount:\n"
                + HelpExampleCli("sendmany",
                                 "\"\" \"{\\\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\":0.02}\" 1 \"\" \"[\\\"1D1ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\",\\\"1353tsE8YMTA4EuV7dgUXGjNFf9KpVvKHz\\\"]\"") +
                "\nAs a json rpc call\n"
...
UniValue listreceivedbyaddress(const UniValue &params, bool fHelp) {
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
                ...
                "    \"amount\" : x.xxx,                  (numeric) The total amount in " + CURRENCY_UNIT +
...
UniValue listaccounts(const UniValue &params, bool fHelp) {
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin();
        ...
        if (nDepth >= nMinDepth) {
            BOOST_FOREACH(const COutputEntry &r, listReceived)if (pwalletMain->mapAddressBook.count(r.destination))
...
UniValue fundrawtransaction(const UniValue &params, bool fHelp) {
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                CURRENCY_UNIT + " per KB)\n"
                                ...
                                "  \"fee\":       n,         (numeric) Fee in " + CURRENCY_UNIT +
    ...
    if (!pwalletMain->FundTransaction(tx, nFeeOut, overrideEstimatedFeerate, feeRate, changePosition, strFailReason,
                                      includeWatching, lockUnspents, changeAddress))
...
UniValue spendzerocoin(const UniValue &params, bool fHelp) {
    ...
    string strError = pwalletMain->SpendZerocoin(thirdPartyaddress, nAmount, denomination, wtx, coinSerial, txHash,
                                                 zcSelectedValue,
                                                 zcSelectedIsUsed);
...
UniValue listmintzerocoins(const UniValue &params, bool fHelp) {
    ...
    BOOST_FOREACH(const CZerocoinEntry &zerocoinItem, listPubcoin) {
                    if (fAllStatus || zerocoinItem.IsUsed ||
                        (zerocoinItem.randomness != 0 && zerocoinItem.serialNumber != 0)) {
                        UniValue entry(UniValue::VOBJ);
    ...
    BOOST_FOREACH(const CZerocoinEntry &zerocoinItem, listPubcoin) {
                    if (zerocoinItem.serialNumber != 0) {
                        ...
                        if (zerocoinItem.serialNumber == coinSerial) {
                            ...
                            zerocoinTx.randomness = zerocoinItem.randomness;
                            pwalletMain->NotifyZerocoinChanged(pwalletMain, zerocoinTx.value.GetHex(),
                                                               zerocoinTx.IsUsed ? "Used" : "New", CT_UPDATED);
                            walletdb.WriteZerocoinEntry(zerocoinTx);
